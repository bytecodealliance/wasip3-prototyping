...
impl Debug for TcpState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::BindStarted => f.debug_tuple("BindStarted").finish(),
            Self::Bound(_) => f.debug_tuple("Bound").finish(),
            Self::Listening(_) => f.debug_tuple("Listening").finish(),
...
impl TcpSocket {
    ...
    pub fn set_keep_alive_enabled(&self, value: bool) -> Result<(), ErrorCode> {
        let fd = self.as_fd()?;
        let v = sockopt::set_socket_keepalive(fd, value)?;
        Ok(v)
    }
...
pub fn bind(
    ...
    if !is_valid_unicast_address(local_address.ip(), socket_family) {
        return Err(ErrorCode::InvalidArgument);
    }
...
pub async fn connect(
    ...
    socket_family: SocketAddressFamily,
) -> Result<tokio::net::TcpStream, ErrorCode> {
    let ip = remote_address.ip().to_canonical();
    if !is_valid_unicast_address(ip, socket_family)
        || ip.is_unspecified()
        || remote_address.port() == 0
    {
        return Err(ErrorCode::InvalidArgument);
    }
    let stream = socket.connect(remote_address).await?;
    Ok(stream)
}
