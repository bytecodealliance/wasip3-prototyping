...
fn build_and_generate_tests() {
    ...
    for target in targets {
        ...
        let kind = match target.as_str() {
            s if s.starts_with("p3_api") => "p3_api",
            s if s.starts_with("p3_cli") => "p3_cli",
            s if s.starts_with("p3_clocks") => "p3_clocks",
            s if s.starts_with("p3_filesystem") => "p3_filesystem",
            s if s.starts_with("p3_http") => "p3_http",
            s if s.starts_with("p3_random") => "p3_random",
            s if s.starts_with("p3_sockets") => "p3_sockets",
        ...
        let adapter = match target.as_str() {
            ...
            s if s.starts_with("p3_cli") => &reactor_adapter,
            s if s.starts_with("p3_clocks") => &reactor_adapter,
            s if s.starts_with("p3_filesystem") => &reactor_adapter,
            s if s.starts_with("p3_http") => &reactor_adapter,
            s if s.starts_with("p3_random") => &reactor_adapter,
            s if s.starts_with("p3_sockets") => &reactor_adapter,
            s if s.starts_with("p3_api_proxy") => &proxy_adapter,
